...
async def build_evidence(doc_id: str, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        with open(document.file_path, "rb") as handle: raw = handle.read()
        text_content = raw.decode("utf-8", errors="ignore")
    except OSError as exc: raise HTTPException(status_code=500, detail=f"Unable to read document source: {exc}") from exc
...
async def compare_agent(request: AgentCompare, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    allowed = allowed_document_ids(db, tenant_id=tenant_id, user_id=current.id, role=current.role); return {"result": compare_documents(request.doc_id_1, request.doc_id_2, request.query, tenant_id=tenant_id, allowed_ids=allowed)}
