...
from app.db.models import Document, DocumentChunk, DocumentPermission, Job, User
    ...
    try:
        ...
        if not duplicate and job.status == "queued":
            try:
                ...
                return {"document_id": document.id, "job_id": job.id, "status": job.status, "checkpoint": job.checkpoint, "queued": False, "error": f"Worker unavailable; job remains durable and can be retried: {exc}"}
...
async def get_job(job_id: str, tenant_id: str = Depends(get_tenant_id), db: Session = Depends(get_db)):
    ...
    return {"job_id": job.id, "document_id": payload.get("document_id"), "type": job.type, "status": job.status, "checkpoint": job.checkpoint, "attempts": job.attempts, "max_attempts": job.max_attempts, "error": job.error, "created_at": job.created_at, "updated_at": job.updated_at}
...
async def build_evidence(doc_id: str, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if settings.use_pgvector:
        chunks = db.query(DocumentChunk).filter(DocumentChunk.doc_id == doc_id, DocumentChunk.tenant_id == tenant_id).order_by(DocumentChunk.chunk_idx).all()
        ...
        text_content = "\n".join(payload.get("documents") or [])
    if not text_content:
        raise HTTPException(status_code=409, detail="Document has not been indexed yet")
    return build_document_graph(db, tenant_id=tenant_id, document_id=doc_id, text=text_content)


@router.get("/evidence/graph")
async def evidence_graph(entity: str | None = None, tenant_id: str = Depends(get_tenant_id), _: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    return graph_snapshot(db, tenant_id=tenant_id, entity=entity)


@router.post("/evidence/verify")
async def evidence_verify(request: ClaimVerification, tenant_id: str = Depends(get_tenant_id), _: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"verification": verify_claims(db, tenant_id=tenant_id, claims=request.claims)}


@router.get("/evidence/contradictions")
async def evidence_contradictions(tenant_id: str = Depends(get_tenant_id), _: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"contradictions": detect_contradictions(db, tenant_id=tenant_id)}
...
async def compare_agent(request: AgentCompare, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    if not can_read_document(db, document_id=request.doc_id_1, tenant_id=tenant_id, user_id=current.id, role=current.role) or not can_read_document(db, document_id=request.doc_id_2, tenant_id=tenant_id, user_id=current.id, role=current.role):
        raise HTTPException(status_code=403, detail="Document access denied")
...
async def report_agent(request: AgentTopic, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"result": generate_report(request.topic, tenant_id=tenant_id, allowed_ids=allowed_document_ids(db, tenant_id=tenant_id, user_id=current.id, role=current.role))}
...
async def bom_agent(request: AgentBOM, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    if not can_read_document(db, document_id=request.doc_id, tenant_id=tenant_id, user_id=current.id, role=current.role):
        raise HTTPException(status_code=403, detail="Document access denied")
...
async def presentation_agent(request: AgentTopic, tenant_id: str = Depends(get_tenant_id), current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"result": generate_presentation(request.topic, tenant_id=tenant_id, allowed_ids=allowed_document_ids(db, tenant_id=tenant_id, user_id=current.id, role=current.role))}
