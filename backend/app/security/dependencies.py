...
def get_current_user(
    ...
    # If a bearer token is supplied, always validate it. This keeps Swagger/API
    # requests tenant-aware even when ENABLE_AUTH is disabled for local development.
    if credentials:
        try:
...
def get_tenant_id(current: CurrentUser = Depends(get_current_user), x_tenant_id: str | None = Header(default=None)) -> str:
    # Never allow a caller-supplied tenant header to override the authenticated
    # tenant. A matching header is tolerated for backwards compatibility.
    if x_tenant_id and x_tenant_id != current.tenant_id:
