...
def _tenant_documents(tenant_id: str, allowed_ids: list[str] | None) -> list[Document]:
    """Load only chunks the caller is authorized to retrieve."""
...
def search_documents(
    ...
    """Search with ACL filtering applied before retrieval and reranking."""
    if top_k < 1:
    ...
    return [
        {"rank": rank, "content": doc.page_content, "metadata": doc.metadata,
         "score": float(score), "retrieval_mode": mode, "reranked": rerank}
