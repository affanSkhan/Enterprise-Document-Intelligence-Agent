...
export default function AgentActions() {
  ...
  const handleRunAgent = async () => {
    ...
    setOutput(null);
    try {
      const data = action === "report"
        ? await api.report(topic)
        : action === "presentation"
          ? await api.presentation(topic)
          : await api.bom(topic);
      setOutput(data.result as AgentOutput);
    } catch (error) {
  ...
  const slides = Array.isArray(output) && output.length > 0 && typeof output[0] === "object" && output[0] !== null && "title" in output[0] ? output as Slide[] : null;
  const bom = Array.isArray(output) && output.length > 0 && typeof output[0] === "object" && output[0] !== null && "part_number" in output[0] ? output as BomItem[] : null;
  ...
  return (
    <div className="bg-neutral-800/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
      ...
      <div className="flex gap-3 mb-8">
        ...
        <button onClick={() => void handleRunAgent()} disabled={!topic.trim() || loading} className="bg-pink-600 hover:bg-pink-500 text-white px-6 py-3 rounded-xl transition-all disabled:opacity-50 font-medium">{loading ? "Running…" : "Generate"}</button>
      </div>
      {output && (
